...
matrix :: Int -> Int -> (Int -> Int -> Int) -> Matrix
matrix m n f = foldl (\accy y -> (foldl (\accx x -> (f y x):accx) [] [1..n]):accy) [] [1..m]

list2Ctor :: Int -> Int -> Int -> Matrix
list2Ctor x y size = matrix size size $ \i j -> x*i+y*j
